...
class ChatApplication:
    ...
    def _setup_main_window(self):
        ...
        # Body parte principal donde mostraremos la conversacion
        self.text_widget = Text(self.window, width=20, height=2, bg=BG_COLOR, fg=TEXT_COLOR, font=FONT, padx=5, pady=5)
        self.text_widget.place(relheight=0.745, relwidth=1, rely=0.08)
        # self.text_widget.configure(cursor="arrow", state=DISABLED)
        
        # scroll para la parte del body 
        scrollbar = Scrollbar(self.text_widget)
        scrollbar.place(relheight=1, relx=0.974)
        scrollbar.configure(command=self.text_widget.yview)
        
        # Footer espacio donde colocaremos la caga de entrada de texto y el boton
        bottom_label = Label(self.window, bg=BG_GRAY, height=80)
        bottom_label.place(relwidth=1, rely=0.825)
        
        # input espacio donde colocaremos el mensaje
        self.msg_entry = Entry(bottom_label, bg="#2C3E50", fg=TEXT_COLOR, font=FONT)
        self.msg_entry.place(relwidth=0.74, relheight=0.06, rely=0.008, relx=0.011)
        self.msg_entry.focus()
        # llamamos el metodo con la tecla enter
